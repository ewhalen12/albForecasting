function X = TrapezoidGCR(eval_f,eval_Jf,x_start,arg1,arg2,t_start,t_stop,timestep)
...
t(1) = t_start;
for n=1:steps
   fprintf('starting time step %i\n', n);
   dt = min(timestep, (t_stop-t(n)));
   x0 = X(:,n);
   t(n+1)= t(n) + dt;
   X(:,n+1)= newtonNdGCR(@(x)fTrap(x,x0),x0,1e-4,1e-8);
end
